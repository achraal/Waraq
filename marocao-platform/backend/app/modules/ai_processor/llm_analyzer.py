...
# ) -> dict:
#     """Helper unique pour garantir une structure de métriques 100% identique partout."""
...
#     return {
#         "model": MODEL_NAME,
#         "confidence_score": data_json.get("confidence"),
#         "extracted_keywords": data_json.get("keywords", []),
#         "detected_language": data_json.get("language", data_json.get("langue", "fr")),
#         "execution_duration_sec": duration,
#         "text_length_chars": len(texte),
#         "text_word_count": len(mots),
#         "is_short_text": len(mots) < 20 if mots else False,
#         "has_uncertainty_keywords": any(w in raison.lower() for w in ["incertain", "doute", "hésite", "ambigu", "inconnu", "pas clair"]),
#         "ollama_total_duration": reponse_ollama.get("total_duration"),
#         "load_duration": reponse_ollama.get("load_duration"),
#         "prompt_tokens": reponse_ollama.get("prompt_eval_count"),
#         "generated_tokens": reponse_ollama.get("eval_count"),
...
#             contient_scan = False  # Initialisation du drapeau scan pour le lot courant
#             texte_lot = ""
#             for idx_page in range(i, min(i + taille_lot, total_pages)):
#                 # Appel unifié : Renvoie un dictionnaire avec texte et métadonnées OCR
#                 res_meta = extraire_texte_page_pdf_avec_meta(path_pdf, page_num=idx_page)
                
#                 # 1. Extraction du texte
#                 txt = res_meta.get("text", "")
#                 texte_lot += f"\n--- Page {idx_page + 1} ---\n" + txt

#                 # 2. Propagation du flag scan si au moins une page du lot est un scan
...
#                 "inspection_method": "FAST_OCR_ONNX_HEADER" if contient_scan else "NATIVE_TEXT_PYMUPDF"
#             })
...
#                 duration=duration,
#                 data_json=data,
#                 reponse_ollama=reponse.json() if 'reponse' in locals() else {},
...
#         return "INCONNU", f"Erreur système : {str(e)}", {}, _construire_metriques(
#             is_scanned=is_scanned,
#             inspection_method=inspection_method,
#             texte=texte,
